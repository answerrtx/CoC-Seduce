...
JSON_DIR        = ROOT / "scenes" / "json_ds"
CSV_DIR         = ROOT / "scenes" / "csv_ds"

# Model override: set to None to use the value from llm_config.json providers.deepseek.dialogue
MODEL_OVERRIDE: str | None = None

# NO_JUDGE_MODE: if True, use scene_no_judge.txt and only process PHY + INV categories
NO_JUDGE_MODE: bool = True
...
def load_configs():
    ...
    ds_cfg  = llm_cfg["providers"]["deepseek"]
    api_key = ds_cfg["api_key"]
    model   = MODEL_OVERRIDE or ds_cfg["dialogue"]
...
# API CALL
# =====================================================================
def call_ds(client: OpenAI, model: str, prompt: str) -> str:
    last_err = None
    ...
    for cat in categories:
        ...
        for skill in skills:
            for setting in target_settings:
                ...
                t0 = time.time()
                try:
                    raw = call_ds(client, model, prompt)
